    ...
    <div className="w-full min-w-[300px] lg:min-w-[420px] h-11 lg:h-12 flex items-center gap-2 rounded-lg border px-2 text-neutral-500 bg-white group focus-within:border-[#ffbf00]">
      <div>
        {isMobile && isSearchPage ? (
          <Link to={"/"} className="flex items-center justify-center h-full p-2 m-1 cursor-pointer group-focus-within:text-[#ffbf00] bg-white rounded-full shadow-md">
            <FaArrowLeft size={20} />
          </Link>
        ) : (
          <button className="flex items-center justify-center h-full px-2 cursor-pointer group-focus-within:text-[#ffbf00]">
            <IoSearch size={20} />
          </button>
        )}
      </div>
